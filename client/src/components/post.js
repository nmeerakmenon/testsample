...
class Post extends Component {
    ...
    getData() {
        apiRequest(this.apiUrl + '/comments/', 'GET').then(handleErrors)
        .then(result => {
    ...
    addComment = (e, replyComment=false, parentComment=null, commentText=null) => {
        ...
        apiRequest(this.apiUrl + "/comments/", "POST", data).then(handleErrors).then(result => {
            if (result.status === 500) {
              console.log(result);
              return;
            }
    ...
    render() {
        ...
        return (
            <div className="content">
                <div className="post">
                    <Paper>
                    Article about Indian History
                    </Paper>
                </div>
                <div>
                    <Input value={this.state.comment}  multiline rows="2" rowsMax="3" placeholder="Type  your  comment" style={{width: "100%"}} onChange={this.editComment}/>
                </div>
                <Button size="small"
